    	 ...
    	 $(function(){
    		...
    		$.ajax({
    			...
    			success:function(data, status, xhr){
    				$('food', data).each(function(index, item){
    					if(name == $('name', item).text()) {
				    		$('#img-tab-1').html('<img src="' + $('image', item).text() + '" alt="full-image">'); //id명은 #으로 가져오고
				    		$('.pro__detl__title').html('<h2>'+ $('name', item).text() +'</h2>'); //class명은 .으로 가져온다.
				    		$('.pro__details').html('<h3> 제조사 : '+ $('maker', item).text() +'</h3><br/>'); 
				    		$('.ingredient__6').html('<p>'+ $('material', item).text()+ '<\p>');
	
				    		var htmlAlergy="";
							for(var i=0; i<alergy.length; i++){ //alergy 정보 표시 
				    			 if($('material', item).text().includes(alergy[i])){
				    				 htmlAlergy += '<li><i class="zmdi zmdi-star">' + alergy[i] +'</i></li>';
				    			 }
				    		}
							$('.feature__list').html(htmlAlergy);
    					}
    				})
    			},
    			error:function(err){
    					...
    					if(name == $('DESC_KOR', item).text()) {
    						Highcharts.chart('chart', {
    			    		    ...
    			    		    series: [{
    			    		        ...
    			    		        data: [{
    			    		            name: '칼로리',
    			    		            y: Number($('NUTR_CONT1', item).text() == 'N/A'? 0 : $('NUTR_CONT1', item).text()),
    			    		            z: 100
    			    		        }, {
    			    		            name: '탄수화물',
    			    		            y: Number($('NUTR_CONT2', item).text() == 'N/A'? 0 : $('NUTR_CONT2', item).text()),
    			    		            z: 100
    			    		        }, {
    			    		            name: '단백질',
    			    		            y: Number($('NUTR_CONT3', item).text() == 'N/A'? 0 : $('NUTR_CONT3', item).text()),
    			    		            z: 100
    			    		        }, {
    			    		            name: '지방',
    			    		            y: Number($('NUTR_CONT4', item).text() == 'N/A'? 0 : $('NUTR_CONT4', item).text()),
    			    		            z: 100
    			    		        }, {
    			    		            name: '당류',
    			    		            y: Number($('NUTR_CONT5', item).text() == 'N/A'? 0 : $('NUTR_CONT5', item).text()),
    			    		            z: 100
    			    		        }, {
    			    		            name: '나트륨',
    			    		            y: Number($('NUTR_CONT6', item).text() == 'N/A'? 0 : $('NUTR_CONT6', item).text()),
    			    		            z: 100
    			    		        }, {
    			    		            name: '콜레스테롤',
    			    		            y: Number($('NUTR_CONT7', item).text() == 'N/A'? 0 : $('NUTR_CONT7', item).text()),
    			    		            z: 100
    			    		        }, {
    			    		            name: '포화지방산',
    			    		            y: Number($('NUTR_CONT8', item).text() == 'N/A'? 0 : $('NUTR_CONT8', item).text()),
    			    		            z: 100
    			    		        }, {
    			    		            name: '트랜스지방',
    			    		            y: Number($('NUTR_CONT9', item).text() == 'N/A'? 0 : $('NUTR_CONT9', item).text()),
    			    		            z: 100
    			    		        }]
    			    		    }]
    			    		});
    						$("#serving_wt").text($('SERVING_WT', item).text());
    						$("#nutr_cont1").text($('NUTR_CONT1', item).text());
    						$("#nutr_cont2").text($('NUTR_CONT2', item).text());
    						$("#nutr_cont3").text($('NUTR_CONT3', item).text());
    						$("#nutr_cont4").text($('NUTR_CONT4', item).text());
    						$("#nutr_cont5").text($('NUTR_CONT5', item).text());
    						$("#nutr_cont6").text($('NUTR_CONT6', item).text());
    						$("#nutr_cont7").text($('NUTR_CONT7', item).text());
    						$("#nutr_cont8").text($('NUTR_CONT8', item).text());
    						$("#nutr_cont9").text($('NUTR_CONT9', item).text());
    					}
    				})
    			},
    			error:function(err){
